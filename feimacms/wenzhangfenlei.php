...
?>
<TITLE>新闻动态--<?php echo $rowNewsFenlei['fenleiname_ziduan']?></TITLE>
...
<?php	
$queryNewsFenlei="select * from hynewsfenlei_table";
$resultNewsFenleiList=mysql_query($queryNewsFenlei);
...
$Page_size=20; 
$result=mysql_query("select * from hynews_table where fenlei_ziduan='$fenleiId'"); 
...
$sql="select * from hynews_table where fenlei_ziduan='$fenleiId' limit $offset,$Page_size"; 
$result=mysql_query($sql); 
while ($row=mysql_fetch_array($result)) { 
?>
<div class="wenzhangList">
<div class="wenzhangListText"><a href="wenzhangxx.php?id=<?php echo $row['id']?>"><?php echo $row['title_ziduan']?></a></div><div class="wenzhangListDate"><?php echo $row['date_ziduan']?></div>
</div>
<?php 
} 
...
if($page!=1){ 
$key.="<a href=\"".$_SERVER['PHP_SELF']."?id=$fenleiId&page=1\">第一页</a> "; //第一页 
$key.="<a href=\"".$_SERVER['PHP_SELF']."?id=$fenleiId&page=".($page-1)."\">上一页</a>"; //上一页 
}else { 
$key.="第一页 ";//第一页 
$key.="上一页"; //上一页 
} 
...
$key.=" <a href=\"".$_SERVER['PHP_SELF']."?id=$fenleiId&page=".$i."\">".$i."</a>"; 
} 
} 
if($page!=$pages){ 
$key.=" <a href=\"".$_SERVER['PHP_SELF']."?id=$fenleiId&page=".($page+1)."\">下一页</a> ";//下一页 
$key.="<a href=\"".$_SERVER['PHP_SELF']."?id=$fenleiId&page={$pages}\">最后一页</a>"; //最后一页 
}else { 
$key.="下一页 ";//下一页 
$key.="最后一页"; //最后一页 
} 
$key.='</div>'; 
?> 
<div class="wenzhangListFenye"><?php echo $key?></div>
</div>
</div>
