...
if (move_uploaded_file($_FILES['upfile']['tmp_name'], $uploadfile))
   { 
	...
	$query="select * from hytopimg_table ";
    $result0=mysql_query($query);
    $row = mysql_fetch_array($result0);
    if($row != null)
      {
        $sql1 = " UPDATE hytopimg_table SET topimg_ziduan='$filename'";
        $result1 = mysql_query($sql1);
		if($result1) echo "<script language=\"JavaScript\">alert(\"图片提交成功！\");window.location.href='../top.php';</script>";
	  }
	
	 else
	 {
		$sql2 = "INSERT INTO hytopimg_table (topimg_ziduan) values ('$filename')";

        $result2 = mysql_query($sql2);

        if($result2) echo "<script language=\"JavaScript\">alert(\"图片提交成功！\");window.location.href='../top.php';</script>";
	 }
