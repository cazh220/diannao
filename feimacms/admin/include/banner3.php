...
$max=$_POST["MAX_FILE_SIZE_3"];
 
if($max<($_FILES['upfile3']['size']))
   echo "<script> alert('图片大于2000000b！');history.back();</script>";
$exname=strtolower(substr($_FILES['upfile3']['name'],(strrpos($_FILES['upfile3']['name'],'.')+1)));//返回文件后缀名
...
if (move_uploaded_file($_FILES['upfile3']['tmp_name'], $uploadfile))
   { 
    ...
    if($row != null)
      {
        $sql1 = " UPDATE hybanner_table SET banner3_ziduan='$filename'";
	 ...
	 {
		$sql2 = "INSERT INTO hybanner_table (banner3_ziduan) values ('$filename')";
